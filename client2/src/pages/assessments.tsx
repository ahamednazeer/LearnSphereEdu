...
export default function Assessments() {
  ...
  const { data: allAssessments = [], isLoading: assessmentsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const coursePromises = courses.map((course: any) => 
        authenticatedApiRequest("GET", `/api/courses/${course.id}/assessments`)
  ...
  const createAssessmentMutation = useMutation({
    mutationFn: async (assessmentData: any) => {
      const response = await authenticatedApiRequest("POST", `/api/courses/${selectedCourseId}/assessments`, assessmentData);
      return response.json();
    },
    onSuccess: (data) => {
